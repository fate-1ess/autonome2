...
async function GET(request) {
    ...
    const requestedSymbols = symbolsParam ? symbolsParam.split(",").map((s)=>s.trim().toUpperCase()).filter(Boolean) : [
        ...DEFAULT_SYMBOLS
    ];
    const uniqueSymbols = Array.from(new Set(requestedSymbols));
    try {
        const prices = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$config$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["IS_SIMULATION_ENABLED"] ? await getSimulatedPrices(uniqueSymbols) : await getLighterPrices(uniqueSymbols);
...
async function getLighterPrices(symbols) {
    ...
    const results = await Promise.all(symbols.map(async (symbol)=>{
        ...
        try {
            ...
            console.error(`[crypto-prices] Failed to fetch market ${symbol}`, error);
            return {
                symbol,
                price: null
            };
...
async function getSimulatedPrices(symbols) {
    ...
    return symbols.map((symbol)=>{
        try {
            ...
            console.error(`[crypto-prices] Simulator missing market ${symbol}`, error);
            return {
                symbol,
                price: null
            };
...
//# sourceMappingURL=%5Broot-of-the-server%5D__8b4f3008._.js.map