...
import useSimStream, { MarketEvent, TradeEventPayload } from "./useSimStream";
...
function normalizeConfidenceRatio(value: number | null | undefined): number | null {
...
export default function SimulatorPanel() {
  ...
  useEffect(() => {
    ...
    const fetchCompletedTrades = async () => {
      ...
      try {
        const response = await fetch(
          `/api/sim/completed-trades?modelId=${encodeURIComponent(accountId)}`,
          { cache: "no-store" },
        );
        ...
        const trades = Array.isArray(data.trades)
          ? (data.trades as Array<Record<string, unknown>>)
              .map((entry) => {
                ...
                const direction =
                  typeof entry.direction === "string" && entry.direction.toUpperCase() === "SHORT"
                    ? "SHORT"
                    : "LONG";

                const notional = typeof entry.notional === "number" && Number.isFinite(entry.notional)
                  ? Math.abs(entry.notional)
                  : null;
                const realizedPnl = typeof entry.realizedPnl === "number" && Number.isFinite(entry.realizedPnl)
                  ? entry.realizedPnl
                  : 0;
                const leverage = typeof entry.leverage === "number" && Number.isFinite(entry.leverage)
                  ? entry.leverage
                  : null;
                const confidence = typeof entry.confidence === "number" && Number.isFinite(entry.confidence)
                  ? entry.confidence
                  : null;
                const parsedClosedAt = typeof entry.closedAt === "string" ? Date.parse(entry.closedAt) : NaN;
                const timestamp = Number.isFinite(parsedClosedAt) ? parsedClosedAt : Date.now();

                const id = typeof entry.id === "string" ? entry.id : `${symbol}-${timestamp}`;
  ...
  const handleStreamEvent = useCallback(
    (event: MarketEvent) => {
      ...
      if (event.type === "trade") {
        ...
        const notional = typeof payload.notional === "number" && Number.isFinite(payload.notional) ? Math.abs(payload.notional) : null;
        const realized = typeof payload.realizedPnl === "number" && Number.isFinite(payload.realizedPnl) ? payload.realizedPnl : 0;
        const leverage = typeof payload.leverage === "number" && Number.isFinite(payload.leverage) ? payload.leverage : null;
        ...
        setCompletedTrades((prev) => {
          if (prev.some((item) => item.symbol === entry.symbol && item.timestamp === entry.timestamp)) {
            return prev;
          }
  ...
  const tradeSummary = useMemo(() => {
    ...
    const longCount = completedTrades.filter((trade) => trade.direction === "LONG").length;
  ...
  const renderPercentMetric = (ratio: number | null | undefined, fractionDigits = 1) => {
  ...
  const analyticsMetrics = useMemo(
    () => [
      { label: "Acct Value", variant: "currency" as const, value: account?.equity ?? null },
      { label: "Avg Trade Size", variant: "currency" as const, value: tradeSummary.avgTradeSize },
      { label: "Median Trade Size", variant: "currency" as const, value: tradeSummary.medianTradeSize },
      { label: "% Long Trades", variant: "percent" as const, value: tradeSummary.percentLong },
      { label: "Expectancy", variant: "currency" as const, value: tradeSummary.expectancy },
      { label: "Avg Leverage", variant: "leverage" as const, value: tradeSummary.avgLeverage },
      { label: "Median Leverage", variant: "leverage" as const, value: tradeSummary.medianLeverage },
      { label: "Max Leverage", variant: "leverage" as const, value: tradeSummary.maxLeverage },
      { label: "Avg Confidence", variant: "percent" as const, value: tradeSummary.avgConfidence },
      { label: "Median Confidence", variant: "percent" as const, value: tradeSummary.medianConfidence },
    ],
          ...
          {tradeSummary.totalTrades > 0 && (
            <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
              <span className="inline-flex items-baseline gap-1">
                <span className="font-semibold text-foreground">Realized PnL:</span>
                {renderCurrencyMetric(tradeSummary.totalRealized)}
              </span>
              <span className="inline-flex items-baseline gap-1">
                <span className="font-semibold text-foreground">Closed Trades:</span>
                <NumberFlow value={tradeSummary.totalTrades} format={{ maximumFractionDigits: 0 }} />
              </span>
            </div>
          )}
        </CardHeader>
        <CardContent>
          {tradeSummary.totalTrades === 0 ? (
            <div className="rounded-xl border border-dashed border-border/60 bg-muted/20 p-6 text-sm text-muted-foreground">
              Close a simulated position to unlock expectancy, leverage, and confidence analytics.
            </div>
          ) : (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {analyticsMetrics.map((metric) => (
                <div key={metric.label} className="rounded-xl border border-border/60 bg-muted/15 p-4 shadow-sm">
