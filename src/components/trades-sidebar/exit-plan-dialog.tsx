...
export function ExitPlanDialog({ selection, onClose }: ExitPlanDialogProps) {
  return (
    <Dialog open={Boolean(selection)} onOpenChange={(open) => !open && onClose()}>
      {selection ? (
        <DialogContent className="max-w-md" style={{ backgroundColor: `${selection.modelColor}15` }}>
          <DialogHeader>
            <DialogTitle className="flex items-center justify-between gap-3 ">
              <span>{selection.position.symbol} Exit Plan</span>
            </DialogTitle>
            <DialogDescription className="text-foreground">
              {selection.modelLabel} · <Badge
                variant="outline"
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="space-y-1">
                <span className=" uppercase tracking-wide">Invalid Condition</span>
