        ...
        {loading ? (
          ...
          <div className="flex items-center justify-center p-8">
            <div className="text-center text-muted-foreground">
              ...
              <p className="text-xs">Open positions will appear here when models create trades.</p>
            </div>
          </div>
        ) : (
          <div>
            {positions.map((modelPos, modelIdx) => {
              ...
              const totalUnrealizedNumeric =
                ...
                modelPos.positions.reduce(
                  (sum, position) => sum + (normalizeNumber(position.unrealizedPnl) ?? 0),
                  0,
                );
              ...
              return (
                <div key={modelPos.modelId} style={{ backgroundColor: `${modelColor}15` }}>
                  ...
                  <div>
                    ...
                    {modelPos.positions.map((position, idx) => {
                      ...
                      const hasExitPlan = Boolean(
                        position.exitPlan?.target || position.exitPlan?.stop || position.exitPlan?.invalidation,
                      );
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center whitespace-nowrap">
                            <span className={`font-bold uppercase ${signal === "LONG" ? "text-green-500" : "text-red-500"}`}>
                              {signal}
                            </span>
                          </div>
...
function renderSymbolIcon(symbol: string) {
  ...
  return <Image src={icon.src} alt={icon.alt} width={16} height={16} />;
}
