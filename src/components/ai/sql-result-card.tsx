...
export function SqlResultCard({
  ...
  const columnHeaders = payload.columns.length
    ...
    : Array.from(
        new Set(
          payload.rows.flatMap((row) => Object.keys(row)),
        ),
      );

  const formattedDuration = useMemo(() => `${payload.durationMs.toFixed(0)} ms`, [payload.durationMs]);
    ...
    >
      <CardHeader className="space-y-4">
        <div className="flex flex-col gap-1">
          <CardTitle className="text-base font-semibold">SQL Analysis Result</CardTitle>
          <CardDescription className="text-sm">
            {payload.rowCount} row{payload.rowCount === 1 ? "" : "s"} • {formattedDuration}
        ...
        <div className="rounded-xl border border-dashed border-border/60 bg-muted/30 p-4 text-sm text-muted-foreground">
          <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground/80">Clarified question</p>
        ...
        {payload.reasoning && (
          <div className="rounded-xl border border-border/40 bg-muted/20 p-4 text-xs text-muted-foreground">
            <span className="block text-[11px] font-semibold uppercase tracking-wide text-muted-foreground/80">Model rationale</span>
      ...
      <CardContent className="space-y-5">
        <div className="rounded-xl border border-border/60 bg-muted/25 p-4 text-xs">
          <div className="flex flex-wrap items-center justify-between gap-2 pb-3">
            <span className="font-semibold uppercase tracking-wide text-muted-foreground">Generated SQL</span>
            <Button
              type="button"
              variant="ghost"
              size="sm"
              ...
              <table className="min-w-full divide-y divide-border text-left text-sm">
                ...
                <tbody className="divide-y divide-border/60">
                  {payload.rows.map((row, rowIndex) => (
                    <tr key={`row-${rowIndex}`} className="odd:bg-background even:bg-muted/40">
                      {columnHeaders.map((column) => (
                        <td
                          key={`${rowIndex}-${column}`}
                  ...
                  {payload.rows.length === 0 && (
                    <tr>
                      <td colSpan={columnHeaders.length} className="px-4 py-8 text-center text-sm text-muted-foreground">
                        No rows returned.
                      </td>
                    </tr>
                  )}
