...
import type { AccountSnapshot, MarketEvent, OrderSide } from "@server/features/simulator/types";
...
export async function handleSimOrder(request: NextRequest) {
  try {
    ...
    const accountId =
      typeof body.accountId === "string" && body.accountId.trim().length > 0 ? body.accountId.trim() : "default";
