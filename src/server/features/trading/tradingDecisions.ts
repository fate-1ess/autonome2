...
const parseDecisionCandidate = (value: unknown, fallbackSymbol?: string): TradingDecision | null => {
  ...
  const confidence = toNumber(
    record.confidence ?? record.confidenceScore ?? record.confidence_percent ?? record.confidence_percentage,
  );
...
const collectDecisionArrays = (raw: Record<string, unknown>): unknown[] => {
  ...
  const candidateKeys = ["decisions", "positions", "actions", "entries", "closedPositions", "signals"];
...
const collectResultArrays = (raw: Record<string, unknown>): unknown[] => {
  const collections: unknown[] = [];
  const candidateKeys = ["results", "executions", "outcomes", "closedPositions"];
