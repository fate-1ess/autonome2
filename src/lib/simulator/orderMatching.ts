import { ExchangeSimulatorOptions, FillDetail, OrderBookSnapshot, OrderExecution, OrderMatchingInput } from "./types";
...
function asTaker(ctx: MatchingContext): OrderExecution {
  ...
  return { fills, averagePrice, totalQuantity, totalFees, status, reason: status === "partial" ? "insufficient book depth" : undefined };
}
...
export function matchOrder(book: OrderBookSnapshot, order: OrderMatchingInput, options: ExchangeSimulatorOptions, rng: RandomSource): OrderExecution {
  ...
  const shouldTake = typeof order.limitPrice === "number" &&
